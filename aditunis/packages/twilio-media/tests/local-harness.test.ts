...
it("round trips synthetic media on loopback", async () => {
  ...
  client.send(JSON.stringify({ event: "start", streamSid: "MZ1", start: { callSid: "CA1" } }));
  client.send(JSON.stringify({ event: "media", streamSid: "MZ1", media: { payload: Buffer.from(encodeMuLaw(new Int16Array([0, 1000, -1000]))).toString("base64") } }));
